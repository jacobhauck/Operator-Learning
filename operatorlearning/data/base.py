...
class OLDataset(torch.utils.data.Dataset):
    ...
    @staticmethod
    def concat(output_file, *input_files):
        """
        Concatenates OLDatasets into a new one
        :param output_file: Name of file to save concatenated dataset in
        :param input_files: Names of files to concatenate
        """
        all_u, all_v = [], []
        all_x, all_y = [], []
        u_disc_ids, v_disc_ids = [], []
        ...
        def dump(dataset):
            u_keys, v_keys = {}, {}
            key_pairs = zip(dataset.u_keys, dataset.v_keys)
            for (u, x, v, y), (u_key, v_key) in zip(dataset, key_pairs):
                all_u.append(u)
                all_v.append(v)
