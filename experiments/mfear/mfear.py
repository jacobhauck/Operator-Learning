...
class MFEARDemoExperiment(mlx.Experiment):
    def run(self, config, name, group=None):
        ...
        for i in range(config['training']['iterations']):
            ...
            optim.zero_grad()
            pred_batch = model(u=in_batch, x_in=grid_batch, x_out=grid_batch)
        ...
        for i in range(100):
            ...
            pred = model(u=source_disc, x_in=grid[None], x_out=grid[None])
        ...
        solution_pred = ol.GridFunction(
            model(u=source[0](grid)[None], x_in=grid[None], x_out=grid[None])[0].detach(),
            x=grid,
