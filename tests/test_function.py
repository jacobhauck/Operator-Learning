...
@pytest.mark.parametrize('d', [1, 2])
def test_scalar_loose(d):
    if d == 1:
        x = torch.tensor([0.05, 0.2, 0.5, 0.8, 0.9])
        y = torch.sin(x)
        base_f = ol.GridFunction(y, x[:, None], x_min=torch.tensor([0.0]), x_max=torch.tensor([1.0]))

        x_rearrange = x[torch.randperm(len(x))]
        test_sort = ol.GridFunction(y, xs=[x_rearrange], is_sorted=False, x_min=base_f.x_min, x_max=base_f.x_max)
        ...
        test_no_sort = ol.GridFunction(y, xs=[x], is_sorted=True, x_min=base_f.x_min, x_max=base_f.x_max)
        ...
        test_both = ol.GridFunction(y, x[:, None], xs=[x], x_min=base_f.x_min, x_max=base_f.x_max)
        ...
        x1 = torch.tensor([0.05, 0.2, 0.5, 0.9])
        x2 = torch.tensor([0.01, 0.1, 0.6, 0.7, 0.8, 0.95])
        x = torch.stack(torch.meshgrid([x1, x2], indexing='ij'), dim=-1)
        y = torch.sin(x[..., 0]) * torch.cos(x[..., 1])
        base_f = ol.GridFunction(y, xs=[x1, x2], is_sorted=True, x_min=torch.zeros(2), x_max=torch.ones(2))

        x1_rearrange = x1[torch.randperm(len(x1))]
        x2_rearrange = x2[torch.randperm(len(x2))]
        test_sort = ol.GridFunction(
            y, xs=[x1_rearrange, x2_rearrange], is_sorted=False,
            x_min=base_f.x_min, x_max=base_f.x_max
        )
        # noinspection PyTypeChecker
        assert torch.all(test_sort.x == base_f.x)
        # noinspection PyTypeChecker
        ...
        test_both = ol.GridFunction(
            y, x=x, xs=[x1, x2], is_sorted=True,
            x_min=base_f.x_min, x_max=base_f.x_max
        )
        # noinspection PyTypeChecker
        assert torch.all(test_both.x == base_f.x)
...
@pytest.mark.parametrize('d', [1, 2])
def test_vector_loose(d):
    if d == 1:
        x = torch.tensor([0.05, 0.2, 0.5, 0.9])
        y = torch.stack([torch.sin(x), torch.cos(x)], dim=-1)
        base_f = ol.GridFunction(y, x[:, None], x_min=torch.tensor([0.0]), x_max=torch.tensor([1.0]))
        ...
        x1 = torch.tensor([0.05, 0.2, 0.5, 0.9])
        x2 = torch.tensor([0.1, 0.6, 0.7, 0.8, 0.95])
        x = torch.stack(torch.meshgrid([x1, x2], indexing='ij'), dim=-1)
        y = torch.stack([
            torch.sin(x[..., 0]) * torch.cos(x[..., 1]),
            torch.cos(x[..., 0]) * torch.sin(x[..., 1])
        ...
        test_meshgrid = ol.GridFunction(y, x=x, x_min=base_f.x_min, x_max=base_f.x_max)
        # noinspection PyTypeChecker
        assert torch.all(test_meshgrid.x == base_f.x)
        # noinspection PyTypeChecker
        assert torch.all(test_meshgrid.xs[0] == base_f.xs[0])
